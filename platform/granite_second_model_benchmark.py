...
def visible(text: str) -> str:
    # Granite may separate private-style reasoning from its response tag.
    m=re.search(r"<response>\s*(.*?)\s*</response>",text,re.I|re.S)
    return m.group(1).strip() if m else re.sub(r"<think>.*?</think>","",text,flags=re.I|re.S).strip()

def norm(text: str) -> str:
    return re.sub(r"[^a-z0-9]+","",visible(text).lower().strip())
...
def main():
    ...
    model=AutoModelForCausalLM.from_pretrained(args.model_id,revision=args.revision,dtype=torch.bfloat16)
    model.eval()
    rows=[]
    for case in CASES:
        rendered=tok.apply_chat_template(
            [
                {"role":"system","content":"Follow the requested output format exactly. Do not explain unless asked."},
        ...
        with torch.no_grad():
            generated=model.generate(**inputs,max_new_tokens=64,do_sample=False,pad_token_id=tok.eos_token_id)
        new=generated[0][inputs["input_ids"].shape[1]:]
        raw=tok.decode(new,skip_special_tokens=True).strip()
        vis=visible(raw); normalized=norm(raw)
        rows.append({**case,"raw_output":raw,"visible_output":vis,"normalized":normalized,"pass":normalized==case["expected"]})
    score=sum(int(r["pass"]) for r in rows)
    out={
      "schema":"CEREBRON_SECOND_MODEL_COLD_BENCHMARK_V1",
      "name":"granite-3.3-2b-instruct","model_id":args.model_id,"revision":args.revision,
      "license":"apache-2.0","runtime":"github-actions-cpu-bfloat16",
      "torch_version":torch.__version__,"transformers_version":transformers.__version__,
      "parameter_count":sum(p.numel() for p in model.parameters()),
      "score":score,"max_score":len(rows),"exact_match_rate":score/len(rows),
      "qualification_threshold":8,"qualified":score>=8,"cases":rows,
      "latency_s":round(time.perf_counter()-started,3),
      "limitations":["tiny exact-output cold benchmark only","not general capability","model-specific response-tag stripping only"],
    }
    out["output_sha256"]=sha(out)
    p=pathlib.Path("platform/artifacts/granite-second-model-benchmark.json")
