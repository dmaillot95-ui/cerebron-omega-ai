...
def main():
    ...
    for case in CASES:
        ...
        with torch.no_grad():
            generated=model.generate(**inputs,max_new_tokens=24,do_sample=False,pad_token_id=tok.eos_token_id)
        new=generated[0][inputs["input_ids"].shape[1]:]
        output=tok.decode(new,skip_special_tokens=True).strip()
        normalized=norm(output)
        rows.append({**case,"output":output,"normalized":normalized,"pass":normalized==case["expected"]})

    score=sum(int(r["pass"]) for r in rows)
    out={
        "schema":"CEREBRON_SECOND_MODEL_COLD_BENCHMARK_V1",
        "name":args.name,"model_id":args.model_id,"revision":args.revision,
        ...
        "limitations":[
            ...
            "qualification permits use only as a secondary independent model candidate",
        ],
    }
    out["output_sha256"]=sha(out)
    p=pathlib.Path("platform/artifacts/smollm17-second-model-benchmark.json")
