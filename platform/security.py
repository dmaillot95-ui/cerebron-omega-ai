...
def _last_audit_hash() -> str:
    ...
    try:
        ...
        return "INVALID"
    if value.get("event_hash"):
        return str(value["event_hash"])
...
def verify_audit_chain(path: pathlib.Path | None = None) -> dict:
    ...
    if not target.exists():
        return {"valid": True, "events": 0, "legacy_events": 0, "last_hash": ZERO_HASH}
    previous = ZERO_HASH
    count = 0
    legacy = 0
    ...
    with target.open("r", encoding="utf-8") as f:
        for lineno, line in enumerate(f, 1):
            ...
            if "event_hash" not in event:
                if chained_started:
                    return {"valid": False, "events": count, "line": lineno, "reason": "LEGACY_EVENT_AFTER_CHAIN"}
            ...
            previous = saved_hash
            count += 1
    return {"valid": True, "events": count, "legacy_events": legacy, "last_hash": previous}
