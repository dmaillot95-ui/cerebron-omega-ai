...
def admissible_state(u, r, h):
    """Necessary exact block-state conditions used in the current H-coupling checkpoint."""
...
def hcouples(a, b):
    """Exact H-coupling from source a=(u_j,r_j,h_j) to b=(u_{j+1},r_{j+1},h_{j+1})."""
    uj, rj, hj = a
    un, rn, hn = b
...
def symbolic_identity(max_r=12):
    ...
    # Exact algebra check for the two equivalent forms of H-coupling.
    if sp:
    ...
    return {
        'worker': 'sympy',
        'claim_level': 'FINITE_SYMBOLIC_VERIFICATION',
        'rows': rows,
...
def z3_hcoupling(umax=8, rmax=10, hmax=3000, max_solutions=500):
    ...
    # Exact bounded search over source and destination block parameters.
        ...
        for un in range(1, umax + 1):
            for rj in range(1, rmax + 1):
                for rn in range(1, rmax + 1):
                    ...
                    s.add(hj % 6 == 5, hn % 6 == 5)
                    # Source/destination u values are exact 3-adic valuations.
                    s.add(((4 ** rj) * hj + 1) % (3 ** uj) == 0)
                    s.add(((4 ** rj) * hj + 1) % (3 ** (uj + 1)) != 0)
                    s.add(((4 ** rn) * hn + 1) % (3 ** un) == 0)
                    s.add(((4 ** rn) * hn + 1) % (3 ** (un + 1)) != 0)
                    # Exact H-coupling. Because 4^rn*hn+1 is odd, this also forces
                    # v2(3^rj*hj+1)=un.
    ...
    return {
        'worker': 'z3',
        'claim_level': 'BOUNDED_SAT_SEARCH',
...
def graph_transitions(umax=7, rmax=10, hmax=1500):
    ...
    # Index destination states by (u_next,r_next), then test the exact coupling.
    by_ur = {}
    for b in states:
        by_ur.setdefault((b[0], b[1]), []).append(b)
    ...
    return {
        'worker': 'graph',
        'claim_level': 'FINITE_GRAPH_SEARCH',
        'bounds': {'u': umax, 'r': rmax, 'h': hmax},
        'state_conditions': ['h>=5', 'h≡5 mod 6', 'v3(4^r h + 1)=u', 'exact H-coupling'],
        'nodes': G.number_of_nodes(),
        'edges': G.number_of_edges(),
        'nontrivial_scc_count': len(scc_nontrivial),
        'self_loops_count': len(self_loops),
        'cycles_count': len(cyc),
        'cycles_preview': [list(map(list, c)) for c in cyc[:20]],
    }
...
def main():
    ...
    a = ap.parse_args()
    fn = {
        'sympy': symbolic_identity,
        'modular': modular_scan,
        'z3': z3_hcoupling,
        'graph': graph_transitions,
    }[a.worker]
    result = fn()
    result['schema'] = 'cerebron-collatz-specialized-compute-v2'
